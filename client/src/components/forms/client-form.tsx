...
export function ClientForm({ 
  ...
  const handleTabChange = async (tabValue: string) => {
    if ((tabValue === "contacts" || tabValue === "locations") && mode === "create" && !autoSavedClientId && !hasAutoSaved) {
      if (!hasCompanyName) {
        toast({
      ...
      // Auto-save the client before switching to contacts or locations
      try {
      ...
      <TabsContent value="locations" className="mt-6">
        <LocationsManager 
          ...
          onLocationChange={(location) => {
            if (location) {
              // Update main client address fields with primary location
              ...
              toast({
                ...
                description: `Client address updated to match primary location: ${location.locationName}`,
              });
            }
          }}
        />
