...
import { Scissors, Plus, Trash2, Play, BarChart3, Package, Clock, Zap, Download, FileText, Table } from "lucide-react";
...
export default function SimpleCuttingOptimizer() {
  ...
  const runOptimization = () => {
    ...
    // Simulate optimization with realistic results
    setTimeout(() => {
      const plans: CuttingPlan[] = [];
      let currentPosition = 0;
      let currentStock = 0;
      
      cutRequests.forEach((req, index) => {
        if (currentPosition + req.length > 12000 || currentStock >= stockItems.length) {
          currentStock++;
          currentPosition = 0;
        }
        
        const usesExistingAngle = index > 0 && cutRequests[index - 1].endAngle === req.startAngle && req.startAngle !== 90;
        
        if (!plans[currentStock]) {
          plans[currentStock] = {
            stockId: `stock-${currentStock}`,
            stockLength: 12000,
            cuts: [],
            wasteLength: 0,
            efficiency: 0
        ...
        currentPosition += req.length;
      });
      
      // Calculate efficiency and waste
      plans.forEach(plan => {
        const totalCutLength = plan.cuts.reduce((sum, cut) => sum + cut.length, 0);
        plan.wasteLength = plan.stockLength - totalCutLength;
        plan.efficiency = (totalCutLength / plan.stockLength) * 100;
      });

      const mockResult: OptimizationResult = {
        plans,
        totalWaste: plans.reduce((sum, plan) => sum + plan.wasteLength, 0),
        totalEfficiency: plans.reduce((sum, plan) => sum + plan.efficiency, 0) / plans.length,
        remnants: plans.filter(p => p.wasteLength > 500).map(p => ({ 
          length: p.wasteLength, 
          materialCode: formData.materialType 
        }))
      };
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Minimize waste with dual-angle cutting optimization</p>
        </div>
      </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="90"
                  />
                  <div className="text-xs text-muted-foreground mt-1">Left side cut</div>
                </div>
                <div>
            ...
            <div className="space-y-2">
              ...
              {stockItems.map((stock) => (
                <div key={stock.id} className="flex justify-between items-center p-2 bg-muted rounded">
                  <span>{stock.length}mm - {stock.materialCode}</span>
                  <Button
                    variant="ghost"
                    size="sm"
            ...
            <CardContent className="space-y-6">
              {viewMode === 'detailed' ? (
                ...
                processPlansForDisplay(optimizationResult.plans).map((planGroup, groupIndex) => (
                  <div key={planGroup.plan.stockId + groupIndex} className="border-2 rounded-lg p-4 space-y-4 bg-white">
                    {/* Header */}
                      ...
                      <div className="relative">
                        ...
                        <div className="relative h-12 bg-gradient-to-r from-slate-300 to-slate-400 border-2 border-slate-500 rounded" style={{ width: '100%' }}>
                          ...
                          {planGroup.plan.cuts.map((cut, cutIndex) => {
                            ...
                            return (
                              <div key={cutIndex} className="absolute top-0 h-full">
                                {/* Cut piece */}
                                <div 
                                  className={`h-full ${cutColor} border border-slate-700 flex items-center justify-center text-white text-xs font-bold rounded-sm relative`}
                                  style={{ 
                                    left: `${leftPercent}%`,
                                    width: `${widthPercent}%`,
                                    minWidth: '20px'
                                ...
                                >
                                  ...
                                  {/* Start angle indicator (left side) */}
                                  {cut.startAngle && cut.startAngle !== 90 && (
                                    <div 
                                      className="absolute left-0 top-0 w-0 h-0 border-l-[12px] border-l-red-500 border-b-[12px] border-b-transparent"
                                      title={`Start angle: ${cut.startAngle}°`}
                                    />
                                  )}

                                  {/* End angle indicator (right side) */}
                                  {cut.endAngle && cut.endAngle !== 90 && (
                                    <div 
                                      className="absolute right-0 top-0 w-0 h-0 border-r-[12px] border-r-red-500 border-b-[12px] border-b-transparent"
                                      title={`End angle: ${cut.endAngle}°`}
                                    />
                                  )}
                                </div>
                          ...
                          {planGroup.plan.wasteLength > 0 && (
                            ...
                            >
                              {((planGroup.plan.wasteLength / planGroup.plan.stockLength) * 100) > 5 && 'WASTE'}
                            </div>
                          )}
                        </div>
                    ...
                    {planGroup.plan.cuts.some(cut => cut.usesExistingAngle) && (
                      <div className="bg-green-50 border border-green-200 rounded p-3">
                        <div className="flex items-center gap-2 text-green-800">
                          ...
                          <span className="font-medium">Angle Optimization Achieved!</span>
                        </div>
                        <div className="text-sm text-green-700 mt-1">
                          This plan uses existing angles to reduce cutting time and improve precision.
                        </div>
                      </div>
                    )}
                  </div>
                ...
                processPlansForDisplay(optimizationResult.plans).map((planGroup, groupIndex) => (
                  <div key={planGroup.plan.stockId + groupIndex} className="border rounded-lg p-4 space-y-3 bg-white">
                    <div className="flex justify-between items-center pb-2 border-b">
