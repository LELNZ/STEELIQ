...
export class DatabaseStorage implements IStorage {
  ...
  async getAIDrawingAnalyses(projectId?: number): Promise<any[]> {
    try {
      // Properly query from database - no mock data
      const analyses = await db.select().from(aiDrawingAnalysis).execute().catch(() => []);
      return analyses;
    } catch (error) {
  ...
  async getAIDrawingAnalysis(id: number): Promise<any | undefined> {
    try {
      ...
      console.error('Failed to get AI drawing analysis:', error);
      return undefined;
    }
  }
  ...
  async createAIDrawingAnalysis(analysis: any): Promise<any> {
    try {
      ...
      console.error('Failed to create AI drawing analysis:', error);
      throw error;
    }
  }
  ...
  async updateAIDrawingAnalysis(id: number, analysis: any): Promise<any> {
    try {
      ...
      console.error('Failed to update AI drawing analysis:', error);
      throw error;
    }
  }
